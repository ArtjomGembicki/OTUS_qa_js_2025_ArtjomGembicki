...
test.describe('BookStore API - Books CRUD (реально работающий)', () => {
  test('Add → Get → Delete (update не работает в API)', async ({ request }) => {
    ...
    const books = new BookService(request as APIRequestContext, config.baseURL!, token);

    // 3. Добавляем первую книгу
    const firstIsbn = EXISTING_ISBNS[0];

    const addResponse: APIResponse = await books.addBook({
      userId: userID,
      isbn: firstIsbn
    });
    ...
    //
    // Оставляю свой вариант в коде закомментированным
    //
    ...
    const getResponse: APIResponse = await books.getBook(firstIsbn);
    expect(getResponse.status()).toBe(200);

    // 5. Удаляем книгу из коллекции пользователя
    const deleteResponse: APIResponse = await books.deleteBook(firstIsbn, userID);
...
test.describe('Get book info - parameterized', () => {
  const isbns: string[] = ['9781449325862', '9781449331818', '9781449337711'];
